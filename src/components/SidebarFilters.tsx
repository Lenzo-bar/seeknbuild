...
export function SidebarFilters({ sections, onRefine, isSearching }: Props) {
  ...
  // Reset values when new sections arrive (new search topic)
  useEffect(() => {
    setValues({})
  }, [sections])
    ...
    return (
      <aside className={styles.sidebar}>
        <div className={styles.empty}>
          {isSearching
            ? <><div className={styles.spinner} /><span>Building filters…</span></>
            : <span className={styles.emptyHint}>Filters appear after you search</span>
          }
        </div>
      </aside>
    )
          ...
          return (
            <div key={section.id} className={styles.section}>
              <button className={styles.sectionHeader} onClick={() => toggleSection(section.id)}>
                ...
                <svg className={`${styles.sectionChevron} ${isCollapsed ? styles.sectionChevronCollapsed : ''}`}
              ...
              {!isCollapsed && (
                <div className={styles.sectionBody}>
                  {section.type === 'checkboxes' && section.options?.map(opt => {
                    const checked = !!((values[section.id] as Record<string, boolean>)?.[opt])
                    return (
                  ...
                  {section.type === 'radio' && section.options?.map(opt => {
                    const checked = values[section.id] === opt
                    return (
                      <label key={opt} className={styles.checkItem}>
                        <input
                          type="radio"
