...
function SortableCard({ card, zone, expandedId, onExpand, onDismiss, onToggleDoc }: {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 50 : undefined, opacity: isDragging ? 0.55 : undefined }}>
      <SearchCard
        card={card} zone={zone}
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={cards.map(c => c.id)} strategy={rectSortingStrategy}>
        <div className={styles.grid} style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
          {cards.map(card => (
            <SortableCard key={card.id} card={card} zone={zone} expandedId={expandedId}
